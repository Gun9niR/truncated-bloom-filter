...
if RUN:
    ...
    for dataset, dname in zip(DATASETS, DATASET_NAMES):
        ...
        rs = RangeSkipping(column_names, column_dtypes, len(group_keys_all[0]),
                            utilities, dataset[0], len(group_keys_all[0][0]))
        ...
        with open('relationalresults/{}.json'.format(dname), 'w') as fp:
            json.dump(results, fp)
        
        if RANGES:
            with open('relationalresults/{}_ranges.json'.format(dname), 'w') as fp:
...
if PLOT:
    ...
    for i, dname in enumerate(DATASET_NAMES):  
        ...
        with open('relationalresults/{}.json'.format(dname), 'r') as fp:
    ...
    # save each subplot
    figsr.savefig("relationalresults/skiprate_vs_cr_all_1e-4_SUBPLOTS.png", bbox_inches='tight')
    figwt.savefig("relationalresults/wastedtime_vs_cr_all_1e-4_SUBPLOTS.png", bbox_inches='tight')
    figql.savefig("relationalresults/querylatency_vs_cr_all_1e-4_SUBPLOTS.png", bbox_inches='tight')
